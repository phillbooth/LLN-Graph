...
describe("BoundaryGraph — validateBoundaries", () => {
  ...
  it("emits LLN-BOUNDARY-001 when denied effect crosses boundary", () => {
    const g = buildBoundaryGraph(
      [
    ...
    assert.ok(diags.some((d) => d.code === "LLN-BOUNDARY-001"));
  });

  it("emits LLN-BOUNDARY-002 when secret crosses non-secret boundary", () => {
    const g = buildBoundaryGraph(
      [
        { boundaryId: "src", boundaryType: "api", trustLevel: "validated", allowedEffects: [], deniedEffects: [] },
    ...
    const diags = validateBoundaries(g);
    assert.ok(diags.some((d) => d.code === "LLN-BOUNDARY-002"));
  });
});
...
describe("DependencyGraph — buildDependencyGraph", () => {
  it("produces LLN-GRAPH-003 for missing dependency", () => {
    const { diagnostics } = buildDependencyGraph([
      { name: "task-a", depends: ["MISSING"] },
    ]);
    assert.ok(diagnostics.some((d) => d.code === "LLN-GRAPH-003"));
  });
...
describe("resolveDependencies", () => {
  ...
  it("returns ok:false and LLN-GRAPH-001 for circular dependency", () => {
    ...
    assert.equal(result.diagnostic?.code, "LLN-GRAPH-001");
  });
});
...
describe("ResourceLifecycleGraph", () => {
  ...
  it("advanceState returns diagnostic for invalid transition", () => {
    ...
    assert.equal(result.ok, false);
    assert.equal(result.diagnostic.code, "LLN-GRAPH-005");
  });
  ...
  it("advanceState returns diagnostic for unknown resource", () => {
    ...
    assert.equal(result.ok, false);
    assert.equal(result.diagnostic.code, "LLN-GRAPH-002");
  });
});
...
describe("CapabilityGraph", () => {
  ...
  it("validateCapabilities returns LLN-CAPABILITY-001 for missing grant", () => {
    ...
    assert.ok(diags.some((d) => d.code === "LLN-CAPABILITY-001"));
  });
});
