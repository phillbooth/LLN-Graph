...
export class GraphBuilder<N, E> {
   ...
   * Throws if schemaVersion does not match "lln.graph.v1".
   */
  static fromJSON<N, E>(json: GraphJSON<N, E>): Graph<N, E> {
    if (json.schemaVersion !== "lln.graph.v1") {
      throw new Error(
        `lln-graph: unsupported graph schemaVersion "${json.schemaVersion}". Expected "lln.graph.v1".`,
      );
    }
