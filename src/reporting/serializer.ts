 ...
 * Validates schemaVersion and throws if it does not match "lln.graph.v1".
 */
export function graphFromJSON<N, E>(json: unknown): Graph<N, E> {
  ...
  if (typed.schemaVersion !== "lln.graph.v1") {
    throw new Error(
      `lln-graph: unsupported graph schemaVersion "${String(typed.schemaVersion)}". Expected "lln.graph.v1".`,
    );
  }
